...
window.setInterval(() => {
    if(state==="play"){
        ...
        for (const index in planets) {
            if(planets.length<=1){break;}
            // console.log(planet)
            // let COMx = 0, COMy = 0, COMmass = 0, newSpeedX = 0, newSpeedY = 0, accX=0,accY=0,accR=0,angle;
            let rX,rY,rR,accR,accX=0,accY=0;
            for(const i in planets){
                if(i!==index){
                    ...
                    rX = planets[i].posX-planets[index].posX
                    rY = planets[i].posY-planets[index].posY
                    rR = Math.pow(rX,2)+Math.pow(rY,2)
                    accR=gravitationConst*planets[i].mass/rR
                    accX+=accR*rX/rR
                    accY+=accR*rY/rR
                }
            }
